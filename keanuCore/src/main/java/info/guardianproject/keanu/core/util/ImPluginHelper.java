...
import info.guardianproject.keanu.core.plugin.ImPluginConstants;
...
public class ImPluginHelper {
    ...
    private HashMap<String,ImPluginInfo> mPluginsInfo;
    private HashMap<String, ImPlugin> mPluginObjects;
    ...
    private ImPluginHelper(Context context) {
        mContext = context;
        mPluginsInfo = new HashMap<>();
        mPluginObjects = new HashMap<>();
    }

    public Collection<ImPluginInfo> getPluginsInfo() {
        if (!mLoaded) {
    ...
    public Collection<ImPlugin> getPluginObjects() {
        if (!mLoaded) {
            loadAvailablePlugins();
        }
        return mPluginObjects.values();
    }
    ...
    public long createAdditionalProvider(String name) {
        ...
        mPluginsInfo.put(pluginInfo.mProviderName,pluginInfo);
        mPluginObjects.put(pluginInfo.mProviderName,plugin);
        return providerId;
    }

    public ImPluginInfo getPluginInfo (String name)
    {
        return mPluginsInfo.get(name);
    }

    public ImPlugin getPlugin (String name)
    {
        return mPluginObjects.get(name);
    }
    ...
    public void loadAvailablePlugins() {
        ...
        for (ResolveInfo info : plugins) {
            ...
            mPluginsInfo.put(providerName, pluginInfo);
            mPluginObjects.put(providerName, plugin);
        }
        mLoaded = true;
    }

    private boolean isPluginDuplicated(String providerName) {
        return mPluginsInfo.containsKey(providerName);
    }
